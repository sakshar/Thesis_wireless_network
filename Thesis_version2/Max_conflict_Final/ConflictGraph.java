    ...
    public void GraphModeling(Deployment deployment,int N)
    {
      /*  for(Target target:deployment.targetArrayList) {
       ...
       for(Target target:deployment.targetArrayList)
       {
           ...
           {
               System.out.println("Non conflicted ..... "+target.targetID+" sen : "+target.sensorOrientations.get(0).SensorId+" ori "+target.sensorOrientations.get(0).Orientation);
               int s1=target.sensorOrientations.get(0).SensorId;
               int key = target.sensorOrientations.get(0).Orientation;
               graphMatrix[s1][s1].totalConflict++;
               if(graphMatrix[s1][s1].edgeIntegerHashMap.get(key)!=null)
               {
                   int p=graphMatrix[s1][s1].edgeIntegerHashMap.get(key).intValue();
                   graphMatrix[s1][s1].edgeIntegerHashMap.put(key,p+1);
               }
               else
               {
                   graphMatrix[s1][s1].edgeIntegerHashMap.put(key,1);
               }
       ...
       for(int i=0;i<numberOfSensors;i++)
       {
           for(int j=0;j<numberOfSensors;j++)
           {
               ...
               if(i==j)
               {
                   System.out.print("Sensor1 "+i);
                   System.out.println("  Total non-conflict : "+graphMatrix[i][j].totalConflict);
