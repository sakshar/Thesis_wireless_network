...
public class Main {
    public static void main(String[] args) {
        int numberOfSensors = 0,numberOfTargets = 0,length = 0;
        double range = 0.0,theta = 0.0;
        ...
        try {
            ...
            File fileSensor = new File("sensorPositions.txt");
            ...
            brSensor.close();
            File fileTarget = new File("targetPositions.txt");
     ...
     /*   while(targetArrayListCopy1.size()!=0&&sensorList.size()!=0)
        {
           ...
           // int min=9999;
            ...
            if(orientationKey>0)
            {
                ...
                System.out.println("Selected sensor "+s1+" orientation "+orientationKey/n+" max conflict "+max);
                    ...
                    while (sensorList.size()!=0)
                    {
                       ...
                       // System.out.println("size "+sensorList.size());
                        /*for(int i=0;i<sensorList.size();i++)
                        {
                           for(int k=0;k<n;k++)
                           {
                               for(Target target:sensorWithOrientationHashMap.get(n * nowSensor + ori).targetArrayList)
                               {
                                   sensorWithOrientationHashMap.get(n*sensorList.get(i).sensorID+k).targetArrayList.remove(target);
                                   sensorWithOrientationArrayList.get(n*sensorList.get(i).sensorID+k).numberOftagetsCovered= sensorWithOrientationHashMap.get(n*sensorList.get(i).sensorID+k).targetArrayList.size();
